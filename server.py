...
model = Transformer()
valid_ext = ['.jpg', '.png']
model.load_state_dict(torch.load(os.path.join('./pretrained_model', 'Shinkai_net_G_float.pth')))
model.eval()
model.cpu()
i = 0
...
def post():
    ...
    try:
        ncoded_string = get_encoded_string(file["img"].split('data:image/png;base64,')[1])
        resp = Response(json.dumps({"img":f'data:image/jpg;base64,{ncoded_string.decode("utf-8")}'}), status=200, mimetype='application/json')
